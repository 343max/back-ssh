...
`on-client` is a wrapper arround ssh that allow the user to run shell commands on the client machine from the server one sshed into.

For example you can from your ssh server you can open a URL on your Mac using

```sh
on-client open https://github.com/
```
